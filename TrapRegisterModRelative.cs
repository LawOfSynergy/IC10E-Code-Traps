...
namespace Code_Traps
{
    ...
    public class TrapRegisterModRelative : TrapOpCode
    {
        public static Variable<int> Divisor = Variables.Int.Named("divisor");
        public static readonly HelpString[] Args = { Offset, Index, Divisor, Fixpoint };

        public TrapRegisterModRelative() : base("trrmod") { }
        ...
        {
            return new LocalTrapOperation(chip, lineNumber, source[1], true, source[2], Utils.Mod())
                .WithError(source[3])
                .WithFixpoint(source[4]);
        }
        ...
        {
            return $"Watches for changes to the register at {Index} after every instruction is completed. If any change occurs, and the new value modulo {Divisor} is equal to {Fixpoint}, " +
