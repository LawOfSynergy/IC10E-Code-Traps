...
namespace Code_Traps
{
    ...
    public class TrapReset : ExtendedOpCode
    {
        ...
        {
            return $"Updates the internal state of the trap(s) initialized at the specified line number(s) to its current observed value, effectively skipping any pending trigger. " +
        ...
        public class TrapResetOperation : Operation
        {
            ...
            public override int Execute(int index)
            {
                ...
                for (int i = 0; i < LineGetters.Count; i++)
                {
                    ...
                    ctx.traps.TryGetValue(line, out var trap);
                    trap?.Reset();
                }
                return index + 1;
            }
