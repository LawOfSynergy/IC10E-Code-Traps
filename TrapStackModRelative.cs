...
{
    // treq jump index fixpoint [error]
    public class TrapStackModRelative : TrapOpCode
    {
        public static Variable<int> Divisor = Variables.Int.Named("divisor");
        public static readonly HelpString[] Args = { Offset, Device, Index, Divisor, Fixpoint };

        public TrapStackModRelative() : base("tsrmod") { }

        public override void Accept(int lineNumber, string[] source)
        {
            if (source.Length != 6) throw new ProgrammableChipException(ProgrammableChipException.ICExceptionType.IncorrectArgumentCount, lineNumber);
        }

        public override Operation Create(ChipWrapper chip, int lineNumber, string[] source)
        {
            return new DeviceTrapOperation(chip, lineNumber, source[1], true, source[2], source[3], Utils.Mod())
                .WithError(source[4])
                .WithFixpoint(source[5]);
        }
        ...
        {
            return $"Watches for changes to the stack of {Device} at {Index} after every instruction is completed. If any change occurs, and the new value modulo {Divisor} is equal to {Fixpoint}, " +
