...
{
    // trgt jump index fixpoint
    public class TrapRegisterGreaterThanAbsolute : TrapOpCode
    {
        public static readonly HelpString[] Args = { Jump, Index, Fixpoint };

        public TrapRegisterGreaterThanAbsolute() : base("trgt") { }
        ...
        {
            return new LocalTrapOperation(chip, lineNumber, source[1], false, source[2], Utils.GreaterThan()).WithFixpoint(source[3]);
        }
        ...
        {
            return $"Watches for changes to the register at {Index} after every instruction is completed. If any change occurs, and the new value is greater than {Fixpoint}, then trap checking will be paused, as if by invoking <color=yellow>tsuspend</color> " +
