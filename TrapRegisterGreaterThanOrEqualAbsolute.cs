...
{
    // trge jump index fixpoint
    public class TrapRegisterGreaterThanOrEqualAbsolute : TrapOpCode
    {
        public static readonly HelpString[] Args = { Jump, Index, Fixpoint };

        public TrapRegisterGreaterThanOrEqualAbsolute() : base("trge") { }
        ...
        {
            return new LocalTrapOperation(chip, lineNumber, source[1], false, source[2], Utils.GreaterThanOrEqual()).WithFixpoint(source[3]);
        }
        ...
        {
            return $"Watches for changes to the register at {Index} after every instruction is completed. If any change occurs, and the new value is greater than or equal to {Fixpoint}, then trap checking will be paused, as if by invoking <color=yellow>tsuspend</color> " +
