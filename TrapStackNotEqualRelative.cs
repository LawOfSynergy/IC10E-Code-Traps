...
{
    // trsne offset dev index fixpoint [error]
    public class TrapStackNotEqualRelative : TrapOpCode
    {
        public static readonly HelpString[] Args = { Offset, Device, Index, Fixpoint, Error };

        public TrapStackNotEqualRelative() : base("trsne") { }
        ...
        {
            var rv = new DeviceTrapOperation(chip, lineNumber, source[1], true, source[2], source[3], Utils.NotEqual()).WithFixpoint(source[4]);
            return source.Length == 5 ? rv.WithError(source[5]) : rv;
        }
        ...
        {
            return $"Watches for changes to the stack of {Device} at {Index} after every instruction is completed. If any change occurs, and the new value is not equal to {Fixpoint} within the specified margin of {Error}, " +
