...
namespace Code_Traps
{
    ...
    public class TrapRegisterEqualAbsolute : TrapOpCode
    {
        public static readonly HelpString[] Args = { Jump, Index, Fixpoint, Error };

        public TrapRegisterEqualAbsolute() : base("treq") { }
        ...
        {
            var rv =  new LocalTrapOperation(chip, lineNumber, source[1], false, source[2], Utils.Equal()).WithFixpoint(source[3]);
        ...
        {
            return $"Watches for changes to the register at {Index} after every instruction is completed. If any change occurs, and the new value is equal to {Fixpoint} within the specified margin of {Error}, " +
