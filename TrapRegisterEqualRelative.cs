...
{
    // trreq offset index fixpoint [error]
    public class TrapRegisterEqualRelative : TrapOpCode
    {
        public static readonly HelpString[] Args = { Offset, Index, Fixpoint, Error };

        public TrapRegisterEqualRelative() : base("trreq") { }
        ...
        {
            var rv = new LocalTrapOperation(chip, lineNumber, source[1], true, source[2], Utils.Equal()).WithFixpoint(source[3]);
