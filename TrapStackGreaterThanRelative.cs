...
{
    // trsgt offset dev index fixpoint
    public class TrapStackGreaterThanRelative : TrapOpCode
    {
        public static readonly HelpString[] Args = { Offset, Device, Index, Fixpoint };

        public TrapStackGreaterThanRelative() : base("tsrgt") { }
        ...
        {
            return new DeviceTrapOperation(chip, lineNumber, source[1], true, source[2], source[3], Utils.GreaterThan()).WithFixpoint(source[4]);
        }
        ...
        {
            return $"Watches for changes to the stack of {Device} at {Index} after every instruction is completed. If any change occurs, and the new value is greater than {Fixpoint}, then trap checking will be paused, as if by invoking <color=yellow>tsuspend</color>" +
